    ...
    try:
        ...
        if not (os.path.exists(filePath)):
            ...
            if(qrEnabled.upper() == "YES"):
                ...
                if(int(CONFIG_KEYS["RENEWAL_DATE"]) < int(tempDate) and (CONFIG_KEYS["COBRANDING"] == "YES")):
                    infoScreen(filePath, tempSelectionList, "happyReading_SchoolLogoNotAvailable", indexReturn)
                elif(int(CONFIG_KEYS["RENEWAL_DATE"]) >= int(tempDate) and (CONFIG_KEYS["COBRANDING"] == "YES")):
                    infoScreen(filePath, tempSelectionList, "happyReading_SchoolLogoIcon", indexReturn)

                elif(int(CONFIG_KEYS["RENEWAL_DATE"]) < int(tempDate) and (CONFIG_KEYS["COBRANDING"] == "NO")):
                    infoScreen(filePath, tempSelectionList, "happyReading_subscriptionRenewalScreenIcon", indexReturn)     
                elif(int(CONFIG_KEYS["RENEWAL_DATE"]) >= int(tempDate) and (CONFIG_KEYS["COBRANDING"] == "NO")):
        ...
        while browser.current_url:
            ...
            if "Successful" in splitString:
                ...
                if(int(CONFIG_KEYS["RENEWAL_DATE"]) < int(tempDate)):
                    infoScreen(filePath, tempSelectionList, "happyReading_subscriptionRenewalScreenIcon", returnIndexValue)
                else:
                    if(os.path.exists(os.path.join(ICONS_ROOT, "happyReading_SchoolLogoIcon.png"))):
                        infoScreen(filePath, tempSelectionList, "happyReading_SchoolLogoIcon", returnIndexValue)
                    else:
                        infoScreen(filePath, tempSelectionList, "happyReading_SchoolLogoNotAvailable", returnIndexValue)
                #break
...
def get_printer_status():
    ...
    if dev is None:
        raise ValueError("Printer not found. Check USB connection and IDs.")
    ...
    try:
        # ESC/POS command: DLE EOT 1 (Get printer status)
        status_command = b'\x10\x04\x01'

        # Send command
        out_ep.write(status_command)

        # Give the printer time to respond
        time.sleep(0.1)

        # Read 1 byte response
        response = in_ep.read(1, timeout=2000)
        print(response)
...
#            return "Cover is open."
        if status_byte & 0b00001000:
            return "Paper is missing."
#        elif status_byte & 0b00100000:
#            return "Printer error (jam, etc)."
        else:
...
def get_printer_status():
    try:
        # ESC/POS command to request status (DLE EOT 1)
        status_command = b'\x10\x04\x01'

        # Send command to printer (using correct OUT endpoint)
        p.device.write(p.out_ep, status_command, timeout=1000)

        # Read 1 byte from IN endpoint
        response = p.device.read(p.in_ep, 1, timeout=1000)
        print(response)

        if not response:
            return "No response received from printer."

        status_byte = response[0]
        print(status_byte)

        # Interpret the bits in the status byte
#        if status_byte & 0b00000100:
#            return "Cover is open."
        if status_byte & 0b00001000:
            return "ERROR: Paper is missing."
#        elif status_byte & 0b00100000:
#            return "Printer error (e.g., cutter jam)."
        else:
            return "Printer is OK."

    except Exception as e:
        return f"ERROR: communicating with printer: {e}"
...
def printPaper(filePath, tempSelectionList, returnIndexValue):
    ...
    if(systemPlatform):
        ...
        printerStatusInFo = ""
        while True:
            printerStatusInFo = get_printer_status()
            sleep(0.5)
            if("No response received from printer." not in printerStatusInFo):
                break

        if ("ERROR" not in printerStatusInFo):
            p.image(filePath)
    
            if((CONFIG_KEYS["COBRANDING"] == "YES") and (int(CONFIG_KEYS["RENEWAL_DATE"]) <= int(getCurrentDate()))):
                if(os.path.exists(os.path.join(ICONS_ROOT, "Cobranding_NoLogoAvailable.png"))):
                    p.image(os.path.join(ICONS_ROOT, "Cobranding_NoLogoAvailable.png"))

            elif((CONFIG_KEYS["COBRANDING"] == "YES") and (int(CONFIG_KEYS["RENEWAL_DATE"]) > int(getCurrentDate()))):
                if(os.path.exists(os.path.join(ICONS_ROOT, "cobranding.png"))):
            ...
            if("englishDash_storyPoem" in filePath):
            ...
            loggedData = "PaperJam"
            window.after(0, lambda: infoScreen("", [], "error_paperError", ""))
...
if __name__ == "__main__":
    ...
    if ((p == "Error") and (systemPlatform)):
        ...
        if((int(CONFIG_KEYS["RENEWAL_DATE"]) < int(tempDate)) and (str(CONFIG_KEYS["RENTAL_OR_SUBSCRIPTION"]) == "RENTAL")):
            print("LOCKED")
